...
<html>
<head>
	<title>Research Hub - Collaborative Research</title>
...
<body>
	...
	<div class="container mt-1">
		...
		<h2 style="color: #923D41;"><span class="fa fa-user-friends"></span> Collaborative Research</h2>
		</br>

		<ul class="nav nav-tabs">
			<li class="nav-item">
				<a class="nav-link active" data-toggle="tab">Collaborative Research</a>
			</li>
	  	</ul>
			...
			<div class="table-responsive">
				<?php
					echo '<table class="table table-hover table-sm">
						...
						$research_list_unpaginated = view_all_user_collaborative_research_fxn($a);
						...
						$research_list = view_all_user_collaborative_research_pagination_fxn($a, $this_page_first_result, $results_per_page);
						...
						if (isset($_GET['searchbutton'])) {
							...
							$research_list = search_collaborative_research_fxn($term, $a);
						}
						...
						if ($research_list) {
							...
							foreach ($research_list as $row) {
								...
								if ($row['published'] == 0) {
									echo "<tr>
										<td><a href='view_collaborative_research.php?edit=$research_id'><i class='btn fa fa-eye'></i></a>". $row['research_topic']."</td>
									</tr>";
								}
								else if ($row['published'] == 1) {
									echo "<tr>
										<td><a href='view_published_research.php?view=$research_id'><i class='btn fa fa-eye'></i></a>". $row['research_topic']."</td>
									</tr>";
								}
							...
							echo 'No collaborative research found';
						}
						
					echo '</table>';
					
					echo (isset($_GET['searchbutton']))? '<div>
					<a class="far fa-arrow-alt-circle-left fa-3x" href="collaborative_research.php"></a> </div>' : '';

					echo (isset($_GET['searchbutton']))? '' : '<ul class="pagination pagination-sm">
						<li class="page-item">
							<a class="page-link" href="collaborative_research.php?page=1">First</a>
						</li>';

						if ($page >= 2) {   
							echo (isset($_GET['searchbutton']))? '' : '<li class="page-item">
								<a class="page-link" href="collaborative_research.php?page='.($page - 1).'">Previous</a>
							</li>';   
						}       
						...
						for ($page_number = 1; $page_number <= $number_of_pages; $page_number++) {   
							if ($page_number == $page) {   
								echo (isset($_GET['searchbutton']))? '' : '<li class="page-item active">
									<a class="page-link" href="collaborative_research.php?page='.$page_number.'">'.$page_number.' </a>
								</li>';
							} else {
								echo (isset($_GET['searchbutton']))? '' : '<li class="page-item">
									<a class="page-link" href="collaborative_research.php?page='.$page_number.'">'.$page_number.' </a>
						...
						if ($page < $number_of_pages) {   
							echo (isset($_GET['searchbutton']))? '' : '<li class="page-item">
								<a class="page-link" href="collaborative_research.php?page='.($page + 1).'">Next</a>
							</li>';   
						}   
						
						echo (isset($_GET['searchbutton']))? '' : '<li class="page-item">
							<a class="page-link" href="collaborative_research.php?page='.$number_of_pages.'">Last</a>
						</li>
					</ul>
					<!-- End of pagination -->
